...
<link rel="stylesheet" href="{{ asset('css/tableMaster2.css') }}">
  ...
  <div class="sp-page-head">
    ...
    <button class="btn btn-primary" onclick="buttonAdd()">+ Add Group Customer</button>
  </div>
...
function loadAll () {
  ...
  $.ajax({
    url: "{!! url('mastergroupcustomerloadall') !!}",
    type: "get",
    async: false,
    data: {
      _token : _token,
  ...
  dataRefresh.forEach((item, i) => {
    ...
    <td style="white-space:nowrap;" class='text-center'>
      <div class="action-buttons-wrap">
          <button data-toggle="tooltip" data-placement="top" title="Menu" class="btn-action-sm btn-action-success" type="button" onclick="buttonEdit('${item.KODEGROUPCUSTSUPP}')"><i class="bi bi-pen"></i></button>
          <button data-toggle="tooltip" data-placement="top" title="Menu" class="btn-action-sm btn-action-danger" type="button" onclick="buttonDelete('${item.KODEGROUPCUSTSUPP}')"><i class="bi bi-trash"></i></button>
      </div>
    </td>
    <td>${item.KODEGROUPCUSTSUPP}</td>
    <td>${item.NAMAGROUPCUSTSUPP}</td>
    </tr>`
  });
...
function buttonEdit (kode) {
  ...
  let _token = $("#_token").val();
  $.ajax({
    url: "{!! url('mastergroupcustomerspdetail') !!}",
    type: "get",
    async: false,
    data: {
      _token : _token,
    ...
    success: function(res) {
      ...
      console.log(res)
      document.getElementById("input_edit_kode").value = res[0].KODEGROUPCUSTSUPP
      document.getElementById("input_edit_nama").value = res[0].NAMAGROUPCUSTSUPP
...
function buttonDelete (kode) {
  ...
  alertify.confirm('Hapus Area', 'Apakah yakin ingin menghapus Group ' + kode + ' ?',
      function() {
        console.log('yes')

        $.ajax({
          url: "{!! url('mastergroupcustomerspdelete') !!}",
          type: "post",
          async: false,
          data: {
            _token : _token,
...
function submitEdit () {
  ...
  $.ajax({
    url: "{!! url('mastergroupcustomerspedit') !!}",
    type: "post",
    async: false,
    data: {
      _token : _token,
...
function submitAdd () {
  ...
  $.ajax({
    url: "{!! url('mastergroupcustomerspadd') !!}",
    type: "post",
    async: false,
    data: {
      _token : _token,
