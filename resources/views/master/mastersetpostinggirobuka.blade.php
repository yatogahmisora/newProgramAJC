...
<div class="container-fluid">
  ...
  <div class="row">
    <div class="col-6 text-left">
      <h1>Master Set Posting Giro Buka</h1>
    </div>
    <div class="col-6 text-right">
      <button type="button" class="btn btn-primary btn-lg " style="height: 60px; " onclick="buttonAdd()"  >Add Posting Giro Buka</button>
    </div>
  </div>
...
<div>
    <div class="col-6 text-right">
        <button type="button" class="btn btn-danger btn-lg " style="height: 60px; width: 100px; " onclick="backToSetPosting()"  >Kembali ke Set Posting</button>
    </div>
</div>
...
<div id="contentContainer" class="container-fluid" style="max-width: 900px">
            ...
            <div class="row">
              ...
              <div class="col-4">
                <div class="form-group">
                  <input type="text" class="form-control" id="input_add_kode">
                </div>
              </div>
              <div class="col-4">
                <div class="form-group">
        ...
        <div class="container-fluid">
          <input type="hidden" name="noUrut" id="input_add_noUrut" value="" />
            ...
            <div class="row">
              ...
              <div class="col-4">
                <div class="form-group">
                  <input type="text" class="form-control" id="input_edit_kode">
                </div>
              </div>
              <div class="col-4">
                <div class="form-group">
...
function loadAll () {
  ...
  $.ajax({
    url: "{!! url('mastersetpostinggirobukaloadall') !!}",
    type: "get",
    async: false,
    data: {
      _token : _token,
...
function buttonEdit (kode) {
  ...
  let _token = $("#_token").val();
  $.ajax({
    url: "{!! url('mastersetpostinggirobukaspdetail') !!}",
    type: "get",
    async: false,
    data: {
      _token : _token,
...
function buttonDelete (kode) {
  ...
  alertify.confirm('Hapus Devisi', 'Apakah yakin ingin menghapus Perkiraan Posting Giro Buka ' + kode + ' ?',
      function() {
        console.log('yes')

        $.ajax({
          url: "{!! url('mastersetpostinggirobukaspdelete') !!}",
          type: "post",
          async: false,
          data: {
            _token : _token,
          ...
          success: function(res) {
            if (res != 1) {
              ...
              console.log(res)
              loadAll()
              alertify.success("Perkiraan Posting Giro Buka telah dihapus");
...
function submitEdit () {
  ...
  $.ajax({
    url: "{!! url('mastersetpostingspedit') !!}",
    type: "post",
    async: false,
    data: {
      _token : _token,
    ...
    success: function(res) {
      ...
      if (res != 1) {
        ...
        console.log(res ,'!')
        // $("#formEdit").modal('toggle')
        alertify.success("Data Posting Giro Buka telah diedit");
        loadAll()
        $("#formEdit").modal('toggle')
      }
...
function submitAdd () {
  ...
  $.ajax({
    url: "{!! url('mastersetpostinggirobukaspadd') !!}",
    type: "post",
    async: false,
    data: {
      _token : _token,
    ...
    success: function(res) {
      ...
      if (res != 1) {
        ...
        console.log(res ,'!')
        // $("#formEdit").modal('toggle')
        alertify.success("Data Posting Giro Buka telah ditambah");
        loadAll()
        $("#form").modal('hide')
      }
...
function loadSelectPerkiraan() {
  ...
  $.ajax({
    url: "{!! url('mastersetpostinggirobukaloadperkiraan') !!}",
    type: "get",
    async: false,
    data: {
      _token: _token,
  ...
  dataRefresh.forEach((item, i) => {
    ...
    rowTable += `<tr>
      ...
      <td>${item.Keterangan}</td>
      <td class="text-center">
        <button class="btn btn-success btn-sm" type="button" onclick="buttonPilihPerkiraan('${item.Perkiraan}')">Select</button>
      </td>
    </tr>`;
  });
...
function loadEditSelectPerkiraan() {
  ...
  dataRefresh.forEach((item, i) => {
    ...
    rowTable += `<tr>
      ...
      <td>${item.Keterangan}</td>
      <td class="text-center">
        <button class="btn btn-success btn-sm" type="button" onclick="buttonEditPilihPerkiraan('${item.Perkiraan}')">Select</button>
      </td>
    </tr>`;
  });
