...
<div class="row mt-4">
      <div class="col-6 text-left">
        <h2 style="margin-top:-85px;">Master Lokasi Barang</h2>
      </div>
...
function loadAll () {
  ...
  $.ajax({
    url: "{!! url('masterlokasibarangloadall') !!}",
    type: "get",
    async: false,
    data: {
      _token : _token,
  ...
  dataRefresh.forEach((item, i) => {
    ...
    <td class="text-center">
      <button class="btn btn-success btn-sm hover-tooltip" data-tooltip='Edit Lokasi Barang' type="button" onclick="buttonEdit('${item.KODELOKASI}')"><i class="bi bi-pen"></i></button>
      <button class="btn btn-danger btn-sm hover-tooltip" data-tooltip='Delete Lokasi Barang' type="button" onclick="buttonDelete('${item.KODELOKASI}')"><i class="bi bi-trash"></i></button>
    </td>
...
function buttonEdit (kode) {
  ...
  let _token = $("#_token").val();
  $.ajax({
    url: "{!! url('masterlokasibarangspdetail') !!}",
    type: "get",
    async: false,
    data: {
      _token : _token,
    ...
    success: function(res) {
      ...
      console.log(res)
      document.getElementById("input_edit_kode").value = res[0].KODELOKASI
...
function submitEdit () {
  ...
  $.ajax({
    url: "{!! url('masterlokasibarangspedit') !!}",
    type: "post",
    async: false,
    data: {
      _token : _token,
    ...
    success: function(res) {
      ...
      if (res != 1) {
        ...
        alertify.success("Data Lokasi Barang telah diedit");
        loadAll() 
