...
function loadAll () {
  ...
  document.getElementById('judulPosting').innerHTML = 'Master Posting Akumulasi'

  $('#tabel').DataTable().destroy();

  $.ajax({
    url: "{!! url('mastersetpostingakumulasiloadall') !!}",
    type: "get",
    async: false,
    data: {
      _token : _token,
  ...
  dataRefresh.forEach((item, i) => {
    ...
    rowTable += `<tr>
    <td class="text-center">
      <button class="btn btn-success btn-sm hover-tooltip" data-tooltip='Edit Posting Akumulasi'type="button" onclick="buttonEdit('${item.Perkiraan}')"><i class="bi bi-pen"></i></button>
      <button class="btn btn-danger btn-sm hover-tooltip" data-tooltip='Delete Posting Akumulasi'type="button" onclick="buttonDelete('${item.Perkiraan}')"><i class="bi bi-trash"></i></button>
    </td>
...
function buttonEdit (kode) {
  ...
  let _token = $("#_token").val();
  $.ajax({
    url: "{!! url('mastersetpostingakumulasispdetail') !!}",
    type: "get",
    async: false,
    data: {
      _token : _token,
...
function buttonDelete (kode) {
  ...
  alertify.confirm('Hapus Perkiraan', 'Apakah yakin ingin menghapus Perkiraan Posting UM Piutang ' + kode + ' ?',
      function() {
        ...
        $.ajax({
          url: "{!! url('mastersetpostingakumulasispdelete') !!}",
          type: "post",
          async: false,
          data: {
            _token : _token,
...
function submitEdit () {
  ...
  $.ajax({
    url: "{!! url('mastersetpostingakumulasispedit') !!}",
    type: "post",
    async: false,
    data: {
      _token : _token,
...
function submitAdd () {
  ...
  $.ajax({
    url: "{!! url('mastersetpostingakumulasispadd') !!}",
    type: "post",
    async: false,
    data: {
      _token : _token,
...
function loadSelectPerkiraan() {
  ...
  $.ajax({
    url: "{!! url('mastersetpostingakumulasiloadperkiraan') !!}",
    type: "get",
    async: false,
    data: {
      _token: _token,
