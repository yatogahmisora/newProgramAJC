...
document.getElementById('judulPosting').innerHTML = 'Master Posting Bank'
  ...
  dataRefresh.forEach((item, i) => {
    ...
    rowTable += `<tr>
    <td class="text-center">
      <button class="btn btn-success btn-sm hover-tooltip" data-tooltip='Edit Posting Bank' type="button" onclick="buttonEdit('${item.Perkiraan}')"><i class="bi bi-pen"></i></button>
      <button class="btn btn-danger btn-sm hover-tooltip" data-tooltip='Delete Posting Bank' type="button" onclick="buttonDelete('${item.Perkiraan}')"><i class="bi bi-trash"></i></button>
    </td>
...
function buttonDelete (kode) {
  ...
  alertify.confirm('Hapus Perkiraan', 'Apakah yakin ingin menghapus Perkiraan Posting Bank ' + kode + ' ?',
      function() {
        ...
        $.ajax({
          url: "{!! url('mastersetpostingbankspdelete') !!}",
          type: "post",
          async: false,
          data: {
            _token : _token,
          ...
          success: function(res) {
            if (res != 1) {
              ...
              console.log(res)
              loadAll()
              alertify.success("Perkiraan Posting Bank telah dihapus");
...
function submitEdit () {
  ...
  $.ajax({
    url: "{!! url('mastersetpostingbankspedit') !!}",
    type: "post",
    async: false,
    data: {
      _token : _token,
  ...
  $.ajax({
    ...
    success: function(res) {
      ...
      if (res != 1) {
        ...
        console.log(res ,'!')
        // $("#formEdit").modal('toggle')
        alertify.success("Data Posting Bank telah ditambah");
        loadAll()
        $("#form").modal('hide')
      }
