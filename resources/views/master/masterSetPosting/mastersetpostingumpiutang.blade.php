...
function loadAll () {
  ...
  document.getElementById('judulPosting').innerHTML = 'Master Posting UM Piutang'

  $('#tabel').DataTable().destroy();

  $.ajax({
    url: "{!! url('mastersetpostingumpiutangloadall') !!}",
    type: "get",
    async: false,
    data: {
      _token : _token,
  ...
  dataRefresh.forEach((item, i) => {
    ...
    rowTable += `<tr>
    <td class="text-center">
      <button class="btn btn-success btn-sm hover-tooltip" data-tooltip='Edit Posting UM Piutang'type="button" onclick="buttonEdit('${item.Perkiraan}')"><i class="bi bi-pen"></i></button>
      <button class="btn btn-danger btn-sm hover-tooltip" data-tooltip='Delete Posting UM Piutang'type="button" onclick="buttonDelete('${item.Perkiraan}')"><i class="bi bi-trash"></i></button>
    </td>
...
function buttonEdit (kode) {
  ...
  let _token = $("#_token").val();
  $.ajax({
    url: "{!! url('mastersetpostingumpiutangspdetail') !!}",
    type: "get",
    async: false,
    data: {
      _token : _token,
...
function buttonDelete (kode) {
  ...
  alertify.confirm('Hapus Perkiraan', 'Apakah yakin ingin menghapus Perkiraan Posting UM Piutang ' + kode + ' ?',
      function() {
        console.log('yes')

        $.ajax({
          url: "{!! url('mastersetpostingumpiutangspdelete') !!}",
          type: "post",
          async: false,
          data: {
            _token : _token,
...
function submitEdit () {
  ...
  $.ajax({
    url: "{!! url('mastersetpostingumpiutangspedit') !!}",
    type: "post",
    async: false,
    data: {
      _token : _token,
...
function submitAdd () {
  ...
  $.ajax({
    url: "{!! url('mastersetpostingumpiutangspadd') !!}",
    type: "post",
    async: false,
    data: {
      _token : _token,
...
function loadSelectPerkiraan() {
  ...
  $.ajax({
    url: "{!! url('mastersetpostingumpiutangloadperkiraan') !!}",
    type: "get",
    async: false,
    data: {
      _token: _token,
