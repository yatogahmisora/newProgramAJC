...
@include('master/partials/sidebarPosting')
  ...
  <div class="row mt-4">
      ...
      <div id='divAdd' class="col-6 text-right" hidden>
...
function loadAll () {
  ...
  document.getElementById('judulPosting').innerHTML = 'Master Posting Hutang Sementara'

  $('#tabel').DataTable().destroy();

  $.ajax({
    url: "{!! url('mastersetpostinghutangsementaraloadall') !!}",
    type: "get",
    async: false,
    data: {
      _token : _token,
  ...
  dataRefresh.forEach((item, i) => {
    ...
    rowTable += `<tr>
    <td class="text-center">
      <button class="btn btn-success btn-sm hover-tooltip" data-tooltip='Edit Posting Hutang Sementara'type="button" onclick="buttonEdit('${item.Perkiraan}')"><i class="bi bi-pen"></i></button>
      <button class="btn btn-danger btn-sm hover-tooltip" data-tooltip='Delete Posting Hutang Sementara'type="button" onclick="buttonDelete('${item.Perkiraan}')"><i class="bi bi-trash"></i></button>
    </td>
...
function buttonEdit (kode) {
  ...
  let _token = $("#_token").val();
  $.ajax({
    url: "{!! url('mastersetpostinghutangsementaraspdetail') !!}",
    type: "get",
    async: false,
    data: {
      _token : _token,
...
function buttonDelete (kode) {
  ...
  alertify.confirm('Hapus Perkiraan', 'Apakah yakin ingin menghapus Perkiraan Posting Hutang Sementara ' + kode + ' ?',
      function() {
        console.log('yes')

        $.ajax({
          url: "{!! url('mastersetpostinghutangsementaraspdelete') !!}",
          type: "post",
          async: false,
          data: {
            _token : _token,
...
function submitEdit () {
  ...
  $.ajax({
    url: "{!! url('mastersetpostinghutangsementaraspedit') !!}",
    type: "post",
    async: false,
    data: {
      _token : _token,
...
function submitAdd () {
  ...
  $.ajax({
    url: "{!! url('mastersetpostinghutangsementaraspadd') !!}",
    type: "post",
    async: false,
    data: {
      _token : _token,
...
function loadSelectPerkiraan() {
  ...
  $.ajax({
    url: "{!! url('mastersetpostinghutangsementaraloadperkiraan') !!}",
    type: "get",
    async: false,
    data: {
      _token: _token,
