...
function loadAll () {
  ...
  document.getElementById('judulPosting').innerHTML = 'Master Posting PPN Masukan'

  $('#tabel').DataTable().destroy();

  $.ajax({
    url: "{!! url('mastersetpostingppnmasukanloadall') !!}",
    type: "get",
    async: false,
    data: {
      _token : _token,
  ...
  dataRefresh.forEach((item, i) => {
    ...
    rowTable += `<tr>
    <td class="text-center">
      <button class="btn btn-success btn-sm hover-tooltip" data-tooltip='Edit Posting PPN Masukan'type="button" onclick="buttonEdit('${item.Perkiraan}')"><i class="bi bi-pen"></i></button>
      <button class="btn btn-danger btn-sm hover-tooltip" data-tooltip='Delete Posting PPN Masukan'type="button" onclick="buttonDelete('${item.Perkiraan}')"><i class="bi bi-trash"></i></button>
    </td>
...
function buttonEdit (kode) {
  ...
  let _token = $("#_token").val();
  $.ajax({
    url: "{!! url('mastersetpostingppnmasukanspdetail') !!}",
    type: "get",
    async: false,
    data: {
      _token : _token,
...
function buttonDelete (kode) {
  ...
  alertify.confirm('Hapus Perkiraan', 'Apakah yakin ingin menghapus Perkiraan Posting PPN Masukan ' + kode + ' ?',
      function() {
        console.log('yes')

        $.ajax({
          url: "{!! url('mastersetpostingppnmasukanspdelete') !!}",
          type: "post",
          async: false,
          data: {
            _token : _token,
...
function submitEdit () {
  ...
  $.ajax({
    url: "{!! url('mastersetpostingppnmasukanspedit') !!}",
    type: "post",
    async: false,
    data: {
      _token : _token,
...
function submitAdd () {
  ...
  $.ajax({
    url: "{!! url('mastersetpostingppnmasukanspadd') !!}",
    type: "post",
    async: false,
    data: {
      _token : _token,
...
function loadSelectPerkiraan() {
  ...
  $.ajax({
    url: "{!! url('mastersetpostingppnmasukanloadperkiraan') !!}",
    type: "get",
    async: false,
    data: {
      _token: _token,
