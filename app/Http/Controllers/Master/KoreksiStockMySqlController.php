...
use App\Model\KoreksiStock;
use App\Model\DetailKoreksiStock;
use App\Model\Nomor;
...
class KoreksiStockController extends Controller {
	...
	public function index() {
        ...
        $menu = Menu::join('akses_menu', 'menu.id', '=', 'akses_menu.id_menu')->where('akses_menu.tampil', 1)->where('menu.grup','>', 0)->where('menu.show_acc', 1)->where('akses_menu.id_user', \Auth::id())->select('menu.*')->get();
			...
			$koreksi = KoreksiStock::join('gudang', 'gudang.kode', '=', 'koreksi_stock.gudang')->whereMonth('koreksi_stock.tanggal', '=', $periode->bulan)->whereYear('koreksi_stock.tanggal', '=', $periode->tahun)->where('koreksi_stock.deleted', 0)->
	...
	public function loadAll() {
		$periode = Periode::where('id_user', \Auth::id())->first();
		return KoreksiStock::join('gudang', 'gudang.kode', '=', 'koreksi_stock.gudang')->whereMonth('koreksi_stock.tanggal', '=', $periode->bulan)->whereYear('koreksi_stock.tanggal', '=', $periode->tahun)->where('koreksi_stock.deleted', 0)->
			orderBy('koreksi_stock.no_bukti', 'DESC')->select('koreksi_stock.*', 'gudang.nama')->get();
	}
	...
	public function generateNomorBukti() {
		...
		if ($nomor->reset == 1) {
			$getCount = KoreksiStock::whereMonth('tanggal', '=', $periode->bulan)->whereYear('tanggal', '=', $periode->tahun)->count();
			$nourut = (string)($getCount + 1);
		} else {
			$getCount = KoreksiStock::whereYear('tanggal', '=', $periode->tahun)->count();
			$nourut = (string)($getCount + 1);
		}
	...
	public function add(Request $req) {
		$count = KoreksiStock::where('no_bukti', $req->input('no_bukti'))->count();
		if ($count == 0) {
			$newKoreksiStock = KoreksiStock::create([
	...
	public function show(Request $req) {
		return KoreksiStock::where('id', $req->input('id'))->first();
	}

	public function showDet(Request $req) {
		return DetailKoreksiStock::where('id_koreksi', $req->input('id'))->get();
	}

	public function edit(Request $req) {
		$getPrev = KoreksiStock::where('id', $req->input('id'))->first();
		$getPrevDet = DetailKoreksiStock::where('id_koreksi', $req->input('id'))->get();
		...
		KoreksiStock::where('id', $req->input('id'))->update([
			'tanggal' => $req->input('tanggal'),
			'gudang' => $req->input('gudang'),
			'keterangan' => $req->input('keterangan')
		]);
		...
		DetailKoreksiStock::where('id_koreksi', $req->input('id'))->delete();
		for ($i = 0; $i < count($req->input('cart')); $i++) {
			DetailKoreksiStock::create([
	...
	public function changeAuth(Request $req) {
		...
		$getKoreksi = KoreksiStock::where('id', $req->input('id'))->first();
	...
	public function erase(Request $req) {
		KoreksiStock::where('id', $req->input('id'))->update(['deleted' => 1]);
		$getPrev = KoreksiStock::where('id', $req->input('id'))->first();
		$getPrevDet = DetailKoreksiStock::where('id_koreksi', $req->input('id'))->get();
		for ($i = 0; $i < count($getPrevDet); $i++) {
			// update stok barang
