...
class MasterSetPostingPiutangSementaraController extends Controller
{
  ...
  public function index(Request $req) {
    ...
    $listData = DB::connection('SML')->select('SELECT a.Perkiraan, b.keterangan from dbposthutpiut a
                       ...
                       where a.Kode=\'PTS\' order by a.Perkiraan');

    $menul0 = app('App\Http\Controllers\NewMenuController')->getMenuL0(2);

    return view('master/masterSetPosting/mastersetpostingpiutangsementara' , [
      "menul0" => $menul0,
      "periode" => $periode,
      "listData" => $listData
    ]);
  ...
  public function spAdd(Request $req)
  {
      ...
      // }
      $listData = DB::connection('SML')->update("INSERT INTO DBPOSTHUTPIUT (Perkiraan, Kode, Tipe, Persen, IsBeliJual, IsLokalorExim, IsUM, Akumulasi, Biaya1, Biaya2, PersenBiaya1, PersenBiaya2) VALUES (:kode,  'PTS', 'L', 0, 0, 0, 0, '', '', '', 0, 0)", ['kode' => $req->kode]);
  ...
  {
      $listData = DB::connection('SML')->update("UPDATE DBPOSTHUTPIUT SET Perkiraan = :kode where Perkiraan = :kodeLama and Kode = 'PTS' ", ['kode' => $req->kode, 'kodeLama' => $req->kodeLama]);
